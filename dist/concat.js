...
function toggleData($this, $iconIndex) {
  var index
  if ($iconIndex === 5) {
    d3.select($this[0].parentNode.parentNode)
      .select('.toggle-text')
      .html(function(d) {
        return (
    ...
    d3.select($this[0].parentNode.parentNode)
      ...
      .html(function(d) {
        if (d.v[dd.musicURL].charAt(0) === '/') {
          return (
            'Music by: <a href=http://www.soundcloud.com' +
            d.v[dd.musicURL] +
            " target='_blank'>" +
            d.v[dd.music] +
...
var statName = ['VIEWS', 'LIKES', 'DISLIKES', 'DURATION']
...
function initLineChart() {
  max = d3.max(chart_data, function(d) {
    return d.v[stat]
  })
...
function resize() {
  ...
  d3.select('path.main-line').attr('d', lineGen(chart_data))
